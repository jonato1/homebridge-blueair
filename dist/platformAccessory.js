...
class BlueAirPlatformAccessory {
    constructor(platform, accessory) {
        ...
        const FakeGatoHistoryService = (0, fakegato_history_1.default)(this.platform.api);
    ...
    async updateDevice() {
        ...
        if ((Date.now() - settings_1.BLUEAIR_DEVICE_WAIT) < this.lastquery) {
    ...
    async updateAccessoryCharacteristics() {
        ...
        const status = await this.updateDevice();
        if (!status) {
    ...
    async updateAirPurifierCurrentAirPurifierState() {
        ...
        let currentValue = this.AirPurifier.getCharacteristic(this.platform.Characteristic.CurrentAirPurifierState).value;
        if (this.accessory.context.attributes.fan_speed > 0) {
    ...
    async updateAirPurifierLockPhysicalControl() {
        ...
        let currentValue = this.AirPurifier.getCharacteristic(this.platform.Characteristic.LockPhysicalControls).value;
    ...
    async updateAirPurifierRotationSpeed() {
        ...
        const fan_speed = this.accessory.context.attributes.fan_speed;
        let currentValue = this.AirPurifier.getCharacteristic(this.platform.Characteristic.RotationSpeed).value;
    ...
    async updateCurrentTemperature() {
        // Current Temperature
        let currentValue = this.TemperatureSensor.getCharacteristic(this.platform.Characteristic.CurrentTemperature).value;
    ...
    async updateCurrentRelativeHumidity() {
        // Current Humidity
        let currentValue = this.HumiditySensor.getCharacteristic(this.platform.Characteristic.CurrentRelativeHumidity).value;
    ...
    async updateCarbonDioxideSensor() {
        ...
        if (this.accessory.context.measurements.co2 !== undefined) {
            // current CO2
            this.CarbonDioxideSensor.updateCharacteristic(this.platform.Characteristic.CarbonDioxideLevel, this.accessory.context.measurements.co2);
            // peak CO2
            const peakco2 = Math.max(...this.accessory.context.devicehistory.co2);
    ...
    async handleAirPurifierActiveSet(state) {
        ...
        if (state === 1) {
            ...
            const url_end = this.accessory.context.uuid + '/attribute/mode/';
    ...
    async handleLockPhysicalControlsSet(state) {
        ...
        const url_end = this.accessory.context.uuid + '/attribute/child_lock/';
...
exports.BlueAirPlatformAccessory = BlueAirPlatformAccessory;
